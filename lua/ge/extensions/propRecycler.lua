...
local function spawnDummyAt(pos, q)
    ...
    if core_vehicles and core_vehicles.spawnNewVehicle then
        local ok, id = pcall(core_vehicles.spawnNewVehicle, "sgentj_dummy", {
            pos = toVec3(pos),
    ...
    if spawn and spawn.spawnVehicle then
        local ok, id = pcall(spawn.spawnVehicle, "sgentj_dummy",
                             {pos = toVec3(pos), rot = quat(qx, qy, qz, qw)})
        if ok and id then return id end
    end
    if be and be.spawnVehicle then
        local ok, id = pcall(function()
            return be:spawnVehicle("sgentj_dummy", toVec3(pos),
                                   quat(qx, qy, qz, qw))
        end)
        if ok and id then return id end
    end
